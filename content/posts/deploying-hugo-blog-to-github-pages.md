...
> Throughout this guide, replace `YOUR-USERNAME` with your GitHub username and `YOUR-REPO-NAME` with the name of your GitHub repository.
...
```bash
hugo version
# Expected: hugo v0.166.0+extended (or later)
```
...
[PaperMod](https://github.com/adityatelange/hugo-PaperMod) is a clean, fast, and feature-rich Hugo theme. Add it as a Git submodule:
...
PaperMod's search feature requires a dedicated page. Create `content/search.md`:
...
Output is written to `public/`. This folder is gitignored — GitHub Actions builds it fresh on every deployment.
...
Deploys artifact to GitHub Pages CDN
       ↓
Site goes live at YOUR-USERNAME.github.io/YOUR-REPO-NAME
```
...
| Wrong URL after deploy | `baseURL` placeholder not updated | Set `baseURL` to `https://YOUR-USERNAME.github.io/YOUR-REPO-NAME/` |
